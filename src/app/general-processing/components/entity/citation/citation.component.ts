...
export class CitationComponent implements OnInit, OnDestroy {
    ...
    openCustomLayout() {
        ...
        this.dialogRef.afterClosed().subscribe(result => {
            this.messageService.setUpdatedLayout(this.eventLayout);
        })
    }
    ...
    changeLayout(type: number) {
        ...
        const obj = { 'layoutType': this.eventLayout.layoutType, 'setLayout': true };
        this.messageService.sendMessage(obj);
    }
    getTicketHistory() {
        this.router.navigateByUrl(`gp/search/entity-details/entity/${this.plateNumber}/citation/${this.currentCitation}/ticket-history`);
    }
    getTicketDetail() {
        this.router.navigateByUrl(`gp/search/entity-details/entity/${this.plateNumber}/citation/${this.currentCitation}/ticket-details`);
    }
    ...
    getData(citationNo: any) {
        this.currentCitation = citationNo;
        ...
        promise
            .then(
                (canvas) => {
                    ...
                    pdf.save("ticket-details.pdf"); // Generated PDF
                }
            )
            .catch(
                (error) => {
