...
@Component({
  selector: 'app-queue-behaviour',
  templateUrl: './queue-behaviour.component.html',
  styleUrls: ['./queue-behaviour.component.scss']
})
export class QueueBehaviourComponent implements OnInit {
  displayedXMLColumns: string[] = ['xmlelement', 'field', 'constantvalue', 'default', 'dateformat', 'action']
  dataSource = new MatTableDataSource<EndBehaviorsModel>();
  xmldataSource = new MatTableDataSource<any>();
  public EndBehviorsForm !: FormGroup;
  ...
  filetransfer: any;
  logoImgBytes: any;
  bahaviorQueueData!: any;


  minDate!: Date;
  behaviorNew: boolean = false;
  ...
  constructor(public translate: TranslateService,
    ...
    private dialogRef: MatDialogRef<QueueBehaviourComponent>,
  ...
  ngOnInit(): void {
    ...
    this.getList();
    this.behaviorNew = this.queueData.behaviorNew
    this.getTransition();
    ...
    this.addFieldForm = new FormGroup({
      'xmlElementPath': new FormControl(''),
    ...
    this.addconstantForm = new FormGroup({
      ...
      'fieldsDescription': new FormControl(''),
    });
  }
  ...
  changeBehaviourType(e: any) {
    ...
    switch (this.behaviorTypesName) {
      ...
      case 'XML Export File':
        ...
        this.EndBehviorsForm.addControl('exportFile', new FormControl(""));
        ...
        this.EndBehviorsForm.addControl('fileTransfersID', new FormControl(""));
    ...
    if (this.addFieldForm.valid) {
      const obj = {
        "xmlExportFieldModel": {
          ...
          "dateTypeFormatId": 1,
          "dateTypeFormatName": data.dateTypeFormatName,
  ...
  addconstants_data(data: any) {
    const obj = {
      "fieldViewModel": {
        "createUserID": 0,
        "updateUserID": 0,
        "createDatetime": "2022-05-19T12:11:08.469Z",
        "updateDatetime": "2022-05-19T12:11:08.469Z",
        "isDeleted": "N",
        "fieldsID": 0,
  ...
  getxmlList() {
    this.apiService.get('XMLExportFile/getXmlExportFieldListByContractId?ContractId=2', true).subscribe(res => {
      this.xmldataSource = res.xmlExportFieldModel;
    })
  }

  deletexmlRecord(id: any) {
    this.apiService.put('XMLExportFile/deleteXmlConstantField?XmlExportFieldId=' + id, id, true).subscribe(res => {
      if (res.status == 'Success') {
        this.getxmlList();
        const msg = "";
  ...
  getTransition() {
    ...
    this.apiService.get('Transitions/getAllTransitions', true).subscribe(resp => {
      if (resp) {
        this.transitionVal = resp.filter((transition: any) => {
          return transition.sourceWorkflowStatesID === this.queueData.workflowStateID
        })
      }
    });
  }
  ...
  getList() {
    this.apiService.get('EndBehaviors/getAllEndBehaviors', true).subscribe(res => {
      if (res) {
        this.dataSource = new MatTableDataSource<EndBehaviorsModel>(res.reverse());
        this.getAllBehaviorTypes();
  ...
  addData(data: any, formDirective:FormGroupDirective) {
    if (this.EndBehviorsForm.valid) {
      const obj = new EndBehaviorsModel(data, this.behaviorTypesName)
      obj.workflowStatesID = this.queueData.workflowStateID;
      switch (this.behaviorTypesName) {
        case "Court Date":
      ...
      this.apiService.post('EndBehaviors/addEndBehaviors', { endBehaviorsModel: obj }, true).subscribe(
        (res) => {
          if (res.status == 'Success') {
            ...
            this.apiService.get(`Queues/GetAllBehaviorsByQueue?QueuesId=${this.queueData.bahaviorQueuesID}`, true).subscribe(res => {
  ...
  editIconClicked(data: any) {
    this.apiService.get(`EndBehaviors/getEndBehaviorsByID?BehaviorsId=${data}`, true).subscribe((res) => {
      this.editData = res;
      this.showForm = true;
      this.showEditForm = true;
      this.behaviorTypesName = res.behaviorTypesName;
      this.addBtn = false;
      switch (this.behaviorTypesName) {
        case "Court Date":
          ...
          this.EndBehviorsForm.addControl('duration', new FormControl(res.courtDateModel.duration));
        ...
        case 'XML Export File':
          ...
          this.EndBehviorsForm.controls['fileTransfersID'].setValue(res.xmlExportFileBehaviorsModel.fileTransfersID.toString());
          break
        case "Printing":
          this.getAllDropDownForPrinting();
          this.conditionalEmailEnable = true;
        ...
        case "Printing":
          ...
          if (this.file != undefined) {
            ...
            obj.printingBehaviorModel = {
              ...
              "fileExtension": "pdf",
              "enableDate": (_.isNull(data.enableDate)) ? new Date() : data.enableDate,
