...
export class ApiService {
    ...
    public apiCW5Url = environment.BASE_CW5_URL;
    public apiV = environment.API_V;
    public adminV = environment.ADMIN_V;
    public violationV1 = environment.SEARCH_V;
    public etimsSearch = environment.BASE_eTIMS_SEARCH_URL;
    ...
    public getViolation(url: string, violationType?: boolean): Observable<any> {
        const hostUrl = (violationType ? this.etimsSearch : this.apiUrl);
        const hostV = (violationType ? this.violationV1 : this.adminV);
        return this.http.get(hostUrl + hostV + url).pipe(map(res => res));
    }
    ...
    public putViolation(url: string, data: any, violationType?: boolean): Observable<any> {
        const hostUrl = (violationType ? this.etimsSearch : this.apiUrl);
        const hostV = (violationType ? this.violationV1 : this.adminV);
